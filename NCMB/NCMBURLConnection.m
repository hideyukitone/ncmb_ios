...
- (NSMutableURLRequest *)createRequest {
    ...
    [request setValue:[NSString stringWithFormat:@"macOS-%@", SDK_VERSION]
   forHTTPHeaderField:kSDKVersionFieldName];
    NSString *osVersion = [[NSProcessInfo processInfo] operatingSystemVersionString];
    [request setValue:[NSString stringWithFormat:@"macOS-%@", osVersion] forHTTPHeaderField:kOSVersionFieldName];
...
- (id)syncConnection:(NSError**)error{
    ...
    //同期通信開始
    //todo
    NSData *contents = [self sendSynchronousRequest:_request
    ...
    - (NSData *)sendSynchronousRequest:(NSURLRequest *)request returningResponse:(NSURLResponse **)response error:(NSError **)error
    {
        ...
        [[[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable _data1, NSURLResponse * _Nullable _response1, NSError * _Nullable _error) {
            resp = _response1;
            err = _error;
            data = _data1;
            reqProcessed = true;
...
- (void)asyncConnectionWithBlock:(NCMBResultBlock)block{
    ...
    //非同期通信開始
    //todo
    [[[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *contentsData,NSURLResponse *response,NSError *connectionError)
     {
         //レスポンス処理
